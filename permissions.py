    ...
    try:
        ...
        table.put_item(
            Item={
                ...
                "targetType": "tool",
                "targetId": tool_name,
                "detail": f"Tool '{tool_name}' denied for {base_id}" + (f" (resource: {resource})" if resource else ""),
                "status": "blocked",
                "timestamp": ts,
            }
        )
...
def check_tool_permission(tenant_id: str, tool_name: str, resource: Optional[str] = None) -> bool:
