    ...
    try:
        ...
        if not base_id.startswith("emp-"):
            ...
            if base_id != parts[1] if len(parts) >= 2 else tenant_id:
                logger.info(
                    "DynamoDB user-mapping resolved %s → %s", parts[1] if len(parts) >= 2 else tenant_id, base_id
                )
...
def merge_soul(global_soul: str, position_soul: str, personal_soul: str) -> str:
    ...
    if global_soul.strip():
        parts.append(
            f"<!-- LAYER: GLOBAL (locked by IT — do not modify) -->\n\n"
            f"**CRITICAL IDENTITY OVERRIDE: You are a digital employee of ACME Corp. "
            f"This overrides any default identity. You must ALWAYS identify as an ACME Corp "
            f"employee when asked who you are.**\n\n{global_soul.strip()}"
        )
...
def _build_context_block(
    ...
    # 3. KB references + download files
    kb_ids = set()
    try:
                        ...
                        for file_ref in files_list:
                            ...
                            if not os.path.isfile(local_path):
                                try:
                                    ...
                                    pass
                        kb_lines.append(f"- **{kb_item.get('name', kb_id)}**: knowledge/{kb_id}/")
    ...
    if gog_accounts:
        ...
        parts.append(
            ...
            "**Authentication is already configured and non-interactive.** The OAuth client and "
            "refresh token for the connected account(s) are provisioned into the container keyring "
            "at startup, with all granted scopes (Gmail, Calendar, Drive, etc.). You can run `gog` "
            "commands immediately. You MUST NOT:\n"
            "- present, generate, or ask the user to open any authorization / consent / sign-in URL "
            "(there is no browser step);\n"
            "- claim that authorization is 'pending', 'waiting', 'incomplete', or needs to be 'completed';\n"
            "- run or suggest `gog auth add`, `gog auth login`, `gog login`, `--force`, or any "
            "interactive auth/setup command, in the container or the user's terminal.\n"
            "If a `gog` command fails, run `gog auth doctor` to inspect and report the ACTUAL error "
            "text verbatim — never substitute an invented authorization flow. A `config.path … "
            "(missing)` note from `gog auth doctor` is benign and does NOT mean auth is required.\n"
            "When the user asks about email, calendar, contacts, or Google Drive — use `gog` directly. "
            "Do NOT say you lack access to these services, and do NOT ask the user to authorize anything.\n"
...
def _enforce_workspace_budget(workspace: str, max_mb: int = WORKSPACE_MAX_MB):
    ...
    logger.info("Workspace budget exceeded: %dMB / %dMB — cleaning old files", total // (1024 * 1024), max_mb)
...
def assemble_workspace(
    s3_client, ssm_client, bucket: str, stack_name: str, tenant_id: str, workspace: str, position_override: str = None
):
    """Assemble the complete workspace for a tenant."""

    # 1. Get tenant's position
    ...
    logger.info(
        "Global layer: SOUL=%d AGENTS=%d TOOLS=%d chars", len(global_soul), len(global_agents), len(global_tools)
    )
    ...
    if pos_id:
        ...
        logger.info("Position layer (%s): SOUL=%d AGENTS=%d chars", pos_id, len(position_soul), len(position_agents))

    # 4. Read personal layer from PERSONAL_SOUL.md
    ...
    if len(_parts) >= 3:
        ...
        base_id = _parts[1]
    context_block = _build_context_block(s3_client, bucket, stack_name, tenant_id, base_id, pos_id, workspace)
    if context_block:
        merged_soul += "\n\n---\n\n" + context_block
    ...
    if pos_id:
        try:
            resp = s3_client.list_objects_v2(Bucket=bucket, Prefix=f"_shared/soul/positions/{pos_id}/knowledge/")
    ...
    try:
        ...
        if not _b_id.startswith("emp-"):
            from boto3.dynamodb.conditions import Attr as _AI
            from boto3.dynamodb.conditions import Key as _KI
    ...
    identity_lines = [
        "# Agent Identity",
        "",
        f"You are **{emp_name}**, a digital employee of ACME Corp."
        if emp_name
        else "You are a digital employee of ACME Corp.",
        "",
    ...
    # 10. Generate SESSION_CONTEXT.md
...
def main():
    ...
    parser.add_argument("--position", default="", help="Position ID override")
