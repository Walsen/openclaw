...
def validate_token(tenant_id: str, resource: str) -> bool:
    """Return True if a valid (non-expired) approval token exists."""
    token: Optional[ApprovalToken] = _token_store.get((tenant_id, resource))

    if token is None:
        logger.info(
            "No approval token found — authorization required tenant_id=%s resource=%s",
            tenant_id,
            resource,
        )
        return False
    ...
    if now >= token.expires_at:
        logger.info(
            "Approval token expired — re-authorization required tenant_id=%s resource=%s expired_at=%s",
            tenant_id,
            resource,
            token.expires_at.isoformat(),
        )
